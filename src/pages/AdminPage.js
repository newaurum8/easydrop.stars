        ...
        return (
            <div className="login-wrapper">
                ...
                <div className="login-card">
                    ...
                    <input type="password" className="modern-input" placeholder="Пароль доступа" value={password} onChange={(e) => setPassword(e.target.value)} style={{marginBottom:'15px', textAlign:'center'}} />
                    <button className="modern-button primary full-width" onClick={() => password === SECRET_PASSWORD ? setIsAuthorized(true) : alert('Неверно')}>Войти в панель</button>
                    <Link to="/" style={{display:'block', marginTop:'20px', color:'#58a6ff', fontSize:'13px', textDecoration:'none'}}>← Вернуться на сайт</Link>
                </div>
            </div>
        );
    }
            ...
            <div className="admin-layout">
                <aside className="admin-sidebar">
                    ...
                    <nav className="sidebar-nav">
                        <button className={`nav-btn ${activeTab === 'items' ? 'active' : ''}`} onClick={() => setActiveTab('items')}>💎 Предметы</button>
                        <button className={`nav-btn ${activeTab === 'cases' ? 'active' : ''}`} onClick={() => setActiveTab('cases')}>🎒 Кейсы</button>
                        <button className={`nav-btn ${activeTab === 'users' ? 'active' : ''}`} onClick={() => setActiveTab('users')}>👥 Пользователи</button>
                    </nav>
                    <div className="sidebar-footer"><Link to="/" className="nav-btn logout">Выйти</Link></div>
                </aside>
...
// ==================================================
// 1. МЕНЕДЖЕР ПРЕДМЕТОВ (НОВЫЙ - С СОЗДАНИЕМ)
// ==================================================
            ...
            <div className="cases-content">
                {(selectedItem || isCreating) ? (
                    <ItemEditor 
                        ...
                        isNew={isCreating}
                    />
                ) : (<div style={{display:'flex', alignItems:'center', justifyContent:'center', height:'100%', color:'#8b949e'}}>Выберите предмет</div>)}
            </div>
        </div>
    );
};
